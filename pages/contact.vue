      ...
      <form netlify ref="contactForm" name="contact" method="POST" class="w-full max-w-lg" enctype="multipart/form-data" netlify-honeypot="extra-field">
        ...
        <div class="flex flex-wrap -mx-3 mb-6">
          <div class="w-full md:w-1/2 px-3 mb-6 md:mb-0">
            ...
            <input v-model="name" :class="errors.name && isSubmitted ? 'border-red-500 dark:border-blue-300' : 'border-gray-200'" class="appearance-none block w-full bg-gray-200 text-gray-700 border py-3 px-4 mb-1 leading-tight focus:outline-none focus:bg-white focus:border-gray-700" name="name" type="text" placeholder="Harold Finch">
            <p v-if="errors.name && isSubmitted" class="text-red-500 dark:text-blue-300 text-xs italic">
              {{ errors.name }}
            </p>
          </div>
          <div class="w-full md:w-1/2 px-3">
            <label class="block uppercase tracking-wide text-gray-700 dark:text-white text-xs font-bold mb-2" for="referer">
              Referer (Optional)
            </label>
            <input v-model="referer" :class="errors.referer && isSubmitted ? 'border-red-500 dark:border-blue-300' : 'border-gray-200'" class="appearance-none block w-full bg-gray-200 text-gray-700 border py-3 px-4 leading-tight focus:outline-none focus:bg-white focus:border-gray-700" name="referer" type="text" placeholder="LinkedIn">
            <p v-if="errors.referer && isSubmitted" class="text-red-500 dark:text-blue-300 text-xs italic">
              {{ errors.referer }}
            </p>
          </div>
        </div>
        <div class="flex flex-wrap -mx-3 mb-6">
          <div class="w-full px-3">
            ...
            <input v-model="email" :class="errors.email && isSubmitted ? 'border-red-500 dark:border-blue-300' : 'border-gray-200'" class="appearance-none block w-full bg-gray-200 text-gray-700 border py-3 px-4 mb-3 leading-tight focus:outline-none focus:bg-white focus:border-gray-700" name="email" type="email" placeholder="harold@ift.us">
            <p v-if="errors.email && isSubmitted" class="text-red-500 dark:text-blue-300 text-xs italic">
              {{ errors.email }}
            </p>
          </div>
        </div>
        <div class="flex flex-wrap -mx-3 mb-2 sm:mb-4">
          <div class="w-full px-3">
            <label class="block uppercase tracking-wide text-gray-700 dark:text-white text-xs font-bold mb-2" for="message">
              Message
            </label>
            <textarea v-model="message" :class="errors.message && isSubmitted ? 'border-red-500 dark:border-blue-300' : 'border-gray-200'" class="appearance-none block w-full bg-gray-200 text-gray-700 border py-3 px-4 mb-3 leading-tight focus:outline-none focus:bg-white focus:border-gray-700 h-24 resize-none" name="message"></textarea>
            <p v-if="errors.message && isSubmitted" class="text-red-500 dark:text-blue-300 text-xs italic">
              {{ errors.message }}
            </p>
          </div>
        </div>
        <div class="sm:flex justify-center sm:justify-between items-center mb-2 sm:mb-6">
          ...
          <div class="py-3 sm:py-0 w-full sm:w-1/3 text-center sm:text-end">
            <button class="bg-gray-700 dark:bg-slate-300 text-white dark:text-gray-700 py-1.5 px-4" type="submit">
              Submit
            </button>
          </div>
        </div>
